		...
		protected String doInBackground(Void... params) {
			...
			try {
				if (operation.equalsIgnoreCase("Connect")) {
					...
					readerModel = ReaderConnect.getModel(reader);
		...
		protected void onPostExecute(String exception) {
			...
			if (!operationStatus) {
				...
				if (operation.equalsIgnoreCase("Connect")) {
					...
					Log.d(TAG, "model="+readerModel);
					if(!readerModel.equalsIgnoreCase(""))
					{
						Log.d(TAG, "Model Set "+readerModel);
						if(!readerModel.equalsIgnoreCase("M6e Nano")
								&& !readerModel.equalsIgnoreCase("M6e Micro")
								&& !readerModel.equalsIgnoreCase("sargas"))
						{
							//展示天线
							antLayout.setVisibility(View.VISIBLE);
							if(readerModel.equalsIgnoreCase("M5e"))
							{
								ant4CheckBox.setEnabled(false);
							}
						}
						else
						{
							ant1CheckBox.setEnabled(true);
							ant2CheckBox.setEnabled(true);
							ant3CheckBox.setEnabled(true);
							ant4CheckBox.setEnabled(true);
